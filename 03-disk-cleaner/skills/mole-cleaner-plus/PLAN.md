...
- **🆕 Smart Folder 引导**：生成智能文件夹，让用户在 Finder 中自主管理
...
- **Smart Folder 选择性输出**：仅对“可视化收益高”的模块（大文件/截图/下载）生成 Smart Folder，其他模块只做报告与路径引导。  
- **风控分级**：硬规则沿用 Mole 的保护机制，软规则输出绿/黄/红建议，避免默认清理导致误删。  
- **推荐动作优先**：默认输出“打开位置/归档建议/Smart Folder”，删除仅在用户显式确认后执行。  
- **扫描策略 × 输出策略**：扫描按时间/大小/类型/位置组合，输出按报告/Smart Folder/打开路径区分，新增模块无需改核心逻辑。  
...
- **Smart Folder 生命周期管理**：为生成的搜索统一命名前缀与归档位置，并提供“清理已生成 Smart Folder”的选项，避免 Saved Searches 堆积。  
...
- 🤔 **决策**：生成 Smart Folder，让用户在 Finder 中自主浏览和选择
- ✅ **行动**：用户可以选择备份到外部存储或删除
...
└────────┬────────┘
         ...
         ▼
┌─────────────────┐
│  3. 生成 Smart   │ → 创建 .savedSearch 智能文件夹
│     Folder      │    按类别：旧视频、旧文档、旧音频
└────────┬────────┘
         │
         ▼
┌─────────────────┐
│  4. 打开 Finder  │ → 自动打开智能文件夹
└────────┬────────┘
         │
         ▼
┌─────────────────┐
│  5. 引导用户     │ → 提示：可以拖拽到外部硬盘备份
│                 │    或者右键删除
└─────────────────┘
```
...
**macOS Smart Folder 原理**：
- 本质是一个 `.savedSearch` 文件（XML 格式）
- 定义搜索条件（大小、类型、日期等）
- 可以用 `open` 命令打开
...
def create_smart_folder(
    name: str,
    min_size_mb: int = 100,
    file_types: list = None,  # ["public.movie", "public.audio", "public.document"]
    older_than_days: int = 365,
    search_path: str = "~"
) -> Path:
    """
    创建一个 macOS Smart Folder (.savedSearch)

    Args:
        name: 智能文件夹名称
        min_size_mb: 最小文件大小 (MB)
        file_types: 文件类型 UTI 列表
        older_than_days: 文件创建时间超过多少天
        search_path: 搜索路径

    Returns:
        生成的 .savedSearch 文件路径
    """
    ...
    query_parts.append(f"kMDItemFSSize >= {min_size_bytes}")

    # 文件类型条件
    if file_types:
        type_conditions = " || ".join([f'kMDItemContentTypeTree == "{t}"' for t in file_types])
        query_parts.append(f"({type_conditions})")

    # 时间条件（创建时间）
    query_parts.append(f"kMDItemFSCreationDate < $time.today(-{older_than_days})")

    # 排除隐藏文件和系统目录
    query_parts.append("kMDItemFSInvisible != 1")

    query_string = " && ".join(query_parts)

    # Smart Folder 的 plist 结构
    search_criteria = {
        "RawQuery": query_string,
        "SearchScopes": [Path(search_path).expanduser().as_posix()],
    }

    # 保存到用户的 Saved Searches 目录
    saved_searches_dir = Path.home() / "Library" / "Saved Searches"
    saved_searches_dir.mkdir(exist_ok=True)

    output_path = saved_searches_dir / f"{name}.savedSearch"

    with open(output_path, "wb") as f:
        plistlib.dump(search_criteria, f)

    return output_path


def open_smart_folder(path: Path):
    """在 Finder 中打开智能文件夹"""
...
if __name__ == "__main__":
    # 创建"超过1年的大视频"智能文件夹
    sf_path = create_smart_folder(
        name="超过1年的大视频",
        min_size_mb=100,
        file_types=["public.movie", "public.mpeg-4", "com.apple.quicktime-movie"],
        older_than_days=365,
        search_path="~"
    )

    print(f"✅ 已创建智能文件夹: {sf_path}")

    # 自动打开
    open_smart_folder(sf_path)
    print("📂 已在 Finder 中打开")
```
...
```bash
# 🥇 核心功能（第一优先级）
python smart_cleaner.py --large-files     # 大文件安全清理 + Smart Folder
...
# Smart Folder 相关
--open-finder      # 自动打开 Finder 显示智能文件夹（默认开启）
--no-open-finder   # 不自动打开 Finder
--keep-smart-folder  # 保留生成的智能文件夹（默认清理）
...
║                                                               ║
╠═══════════════════════════════════════════════════════════════╣
║  [1] 打开大文件Smart Folder  [2] 查看详情  [3] 导出  [Q] 退出 ║
╚═══════════════════════════════════════════════════════════════╝
```
